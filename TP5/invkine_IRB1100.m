function qq = invkine_IRB1100(R,T,q_mejor,q0)
%%
offsets = R.offset;
R.offset = zeros(6,1);
...
for i=1:2
    ...
    beta = atan2(y1,x1);
    r = sqrt(x1^2 + y1^2);
    alfa = acos((a2^2 + r^2 - a3^2 - a4^2)/(2*r*a2));
    
    QQ(2,1+4*(i-1)) = beta - alfa;
    QQ(2,3+4*(i-1)) = beta + alfa;
    ...
    for j =1:2
        ...
        phi = atan2(y2,x2);
        alpha = atan2(a3,a4);
        QQ(3,1+2*(j-1)+4*(i-1)) = phi + alpha - pi/2 ;% (-1)^(i+1))*;+(1-((R.alpha(1,3)<0)*2))*pi/2; 
...
for i=1:2:7
    ...
    for j=i:i+1
        ...
        QQ(5,j) = atan2(T64(2,3), T64(1,3)) - pi/2 ;%+(1-((R.alpha(1,5)<0)*2))*pi/2;
...
R.offset = offsets;
qq = qq - R.offset'*ones(1,length(qq(1,:)));
end